...
namespace Firmware_Patch
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                Usage();
                Console.WriteLine("Options not valid, or not supported");
                Console.WriteLine();
                return;
            }
        ...
        static Firmware FirmwareMatch(string file_name)
        {
            ...
            byte[] d7000_0103Hash = { 0x60, 0x6C, 0x50, 0x98, 0xC4, 0x41, 0x4A, 0x91, 0x98, 0x47, 0x83, 0x3D, 0xAC, 0x45, 0x63, 0x2C };
            byte[] d7000_0102Hash = { 0x10, 0x16, 0x78, 0x3D, 0x86, 0x15, 0xFF, 0x61, 0x70, 0xFA, 0xA9, 0x92, 0x60, 0x6A, 0x89, 0xE1 };
            byte[] d7000_0101Hash = { 0x8F, 0x34, 0xC7, 0x53, 0xBF, 0x51, 0x99, 0xFA, 0x63, 0x06, 0x63, 0xE3, 0xC1, 0x2D, 0x58, 0x76 };
            byte[] d800_0102Hash = {0xFD, 0xD8, 0x91, 0xE9, 0xFC, 0xFE, 0xAF, 0x30, 0x44, 0x74, 0xE2, 0xDC, 0x72, 0x43, 0x70, 0x44 } ;
            byte[] d600_0101Hash = {0x74, 0x0B, 0x38, 0x22, 0x33, 0x58, 0x17, 0xAC, 0xA6, 0x46, 0xF8, 0xB9, 0x2C, 0x3C, 0xF6, 0xC9 } ;
            byte[] d3200_0101Hash = {0xAB, 0x1C, 0x12, 0x9D, 0x37, 0xDC, 0x53, 0xF6, 0x92, 0x60, 0xA4, 0x53, 0xD5, 0x95, 0x62, 0x02 } ;
            byte[] d4_0105Hash =  {0xB0, 0x1F, 0xD3, 0xDF, 0xE5, 0x74, 0x2F, 0xB7, 0x49, 0xA0, 0x85, 0xD3, 0xE4, 0x14, 0x52, 0x7D } ;
            ...
            if (HashSame(hash, d7000_0103Hash))
            {
                return new D7000_0103(file_name);
            }
            if (HashSame(hash, d800_0102Hash))
            {
                return new D800_0102(file_name);
            }
            if (HashSame(hash, d600_0101Hash))
            {
                return new D600_0101(file_name);
            }
            if (HashSame(hash, d3200_0101Hash))
            {
                return new D3200_0101(file_name);
            }
            if (HashSame(hash, d4_0105Hash))
            {
                return new D4_0105(file_name);
            }
            return null;
        }
